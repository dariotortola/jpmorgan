...
public class StockService {
    ...
    /**
     * @param companyId
     * @return all share index of company with the id as geometric mean of their
     *         stock prices
     */
    public BigDecimal getAllShareIndex(int companyId) {
        ...
        default:
            return getAllShareIndex(stocks).setScale(mathContext.getPrecision(), mathContext.getRoundingMode());
        }
    }
     ...
     * @return all share index, as geometric mean
     */
    private BigDecimal getAllShareIndex(List<Stock> stocks) {
        int n = stocks.size();
        double nthRoot = 1 / (double) n;
        Iterator<Stock> iterator = stocks.iterator();
        // at least two elements
        BigDecimal total = BigDecimal.valueOf(Math.pow(getStockPrice(iterator.next()).doubleValue(), nthRoot));
        while (iterator.hasNext()) {
            // pow-ing each element we avoid the risk of overflowing
            total = total.multiply(BigDecimal.valueOf(Math.pow(getStockPrice(iterator.next()).doubleValue(), nthRoot)));
        }
        return total;
    }
    ...
    public BigDecimal getPeRatio(Stock stock) {
        ...
        return tickerPrice.multiply(tickerPrice)
                .divide(new BigDecimal(getDividendBase(stock)), mathContext);
    }
    ...
    public BigDecimal getStockPrice(Stock stock) {
        ...
        if (recent.isEmpty()) {
            ...
            return new BigDecimal(totalPrice).divide(new BigDecimal(totalAmount), mathContext);
        }
    }
